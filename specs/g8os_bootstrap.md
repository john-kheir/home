...
- core0 starts network stack
    ...
    - if no internet or no dhcp server
        - ??? #TODO specify how done today
- core0 components on stick or VM image
    - has IPFS components
    - has our fuse FS component
- check on USB stick or VM image
    - bootstrap.yaml has config info for boot process
    - $githuburl is specified in bootstrap.yaml
    - some to be defined authorization mechanism specified: encr key, ssh key?, ... to be able to download the bootstrap?.yaml #TODO *1
- core0 reads bootstrap?.yaml file from github url
...
- core0 checks hash of own booted core0 with hash of the ones specified in the bootstrap...yaml file
    - if hash different, get the new core0 components from IPFS (hash(es) are known)
    - core0 reloads itself


### core0 (after update)

- mounts the plist as specified above on /? and chroot to it?  (is this true?)
